...
cpp1_t ccp1_obj = {
  ...
  .ccp1_mode_variant = CCP1_CAPTURE_MODE_1_RISING_EDGE,
};
timer3_t timer3_obj = {
...
int ccp1_capture_test(void)
{
    ...
    ret |= lcd_4bit_send_string_pos(&lcd_1, 1, 1, (uint8 *)"CCP1 Capture test");
    __delay_ms(2000);
...
void CCP1_Interrupt_Handler(void)
{
    ...
    {
        (void)ccp1_capture_mode_read_value((uint16 *)&captured_value);
